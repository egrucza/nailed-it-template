...
function App() {
  ...
  // Bee Movie
  function navigatePastBees() {
    navigate("/thanks", { replace: true });
    setIsBeeMovieCheck(false);
    setIsBeeMovieButtonActive(false);
  }

  function beeMovieCheck() {
    setIsBeeMovieCheck(true);
  }

  function scrollBees() {
    const textContent = document.querySelector<HTMLElement>(".bees__text-content");
    const scrollbar = document.querySelector<HTMLElement>(".bees__text-scrollbar__scroller");
    ...
    if (textContent) {
      ...
      var newValue = parseInt(top, 10) - 5;

      if (-newValue <= maxHeight) {
        const newTopStyle = `${newValue}px`;
        textContent.style.top = newTopStyle;
      } else {
        setIsBeeMovieButtonActive(true);
      }
    }

    if (scrollbar) {
      const style = window.getComputedStyle(scrollbar);
      const top = style.getPropertyValue('top').slice(0, -2);
      var newValue = parseFloat(top) + 0.1;
      if (newValue <= 250) {
        const newTopStyle = `${newValue}px`;
        scrollbar.style.top = newTopStyle;
      }
    }
  }

  const beeMovieModalTitle = "Please read through The Bee Movie script before continuing.";
  const beeMovieModalContent = (
    <>
      <div className="bees__scroll-area" onWheel={scrollBees}>
        <div className="bees__text-content">
          <p className="bees__text-content--script">
        ...
        <div className="bees__text-scrollbar">
          <div className="bees__text-scrollbar__scroller">
          </div>
        </div>
      </div>

      <div className="bees-continue">
        {isBeeMovieButtonActive && (
          <button
            className="bees-continue-button"
            onClick={navigatePastBees}>
            Continue
          </button>
        )}
      </div>
  ...
  return (
    <div className="app">
      ...
      {isBeeMovieCheck &&
        <Modal
          title={beeMovieModalTitle}
          content={beeMovieModalContent}
          setIsOpen={isBeeMovieCheck}
          wide={true}
        />
      }
      <footer>
